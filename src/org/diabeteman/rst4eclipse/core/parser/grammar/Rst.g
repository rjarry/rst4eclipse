...
section_title
  : (SPECIAL_CHAR+ NEWLINE)? TEXT NEWLINE SPECIAL_CHAR+ NEWLINE
  ;
...
transistion
  : BLANK_LINE SPECIAL_CHAR SPECIAL_CHAR SPECIAL_CHAR SPECIAL_CHAR SPECIAL_CHAR* NEWLINE BLANK_LINE
  ;
...
directive
  : directive_name WS? directive_block?
  ;

directive_name
  : DOUBLE_DOT ' ' IDENTIFIER DOUBLE_COLUMN
  ;
...
comment_block
  : DOUBLE_DOT (~(NEWLINE))* NEWLINE (INDENT (~(NEWLINE))* NEWLINE)*
  ;
...
@init {int spaces = 0;}
  :  {startPos==0}? =>
      (
        ...
        ( ('\r')? '\n' {if (state.token!=null) state.token.setChannel(99); else $channel=HIDDEN;} )*
           // {token.setChannel(99); }
      )
  ;
